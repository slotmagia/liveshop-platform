...
├─ liveshop-protocol/    # 各模块线协议公共仓；每模块只改自己的子目录
...
├─ liveshop-platform/    # Registry/Navigation/Settings/Audit 控制面、Host SDK
...
| `liveshop-protocol` | 各模块 gRPC 与 Platform contracts 的唯一事实源；Identity/Catalog/Trade/Live/Platform 只改自己的子目录 | 业务实现、数据库、前端 | 各子目录 `tools/generate-proto.ps1` 与公共仓 CI |
...
- 业务仓只含 `business/`（实现与前端贡献）。线协议在公共仓 `liveshop-protocol/<module>/`（独立 Go module）。依赖方向单向：`business` 依赖对应 protocol 子目录的发布产物，protocol 绝不依赖 `business`。
- `liveshop-protocol` 按所有者分子目录：`identity/`、`catalog/`、`trade/`、`live/`、`platform/`。每个子目录只放该模块拥有的线协议：`proto/<module>/v1`、生成客户端、以及 Platform 的 Schema 与域中立契约类型。禁止把其他模块的 Proto 塞进自己的子目录。一次提交只改一个 owner 子目录。
- 不拥有任何线协议的技术组件（如 Gateway 数据面）不得建立 `protocol/`，只能依赖公共仓里别人发布的契约。
...
- 只声明当前 artifact 直接使用的依赖。Host Runtime 不是业务模块依赖；业务模块通过 `@liveshops/host-sdk` 接入。
...
- 总后台与商户后台共用同一套后台界面：Admin/Merch Host、Platform contribution 和所有模块的 `frontend-admin`、`frontend-merch` 一律导入 `@liveshops/design-tokens/console.css`；命令式页面用该包的组件工厂构建，React 页面用 Tailwind 工具类；领域目录只保留领域特有布局。
...
- React + Tailwind 的后台在 `console.css` 之后追加 `@liveshops/design-tokens/tailwind.css`，Tailwind 配置只写 `presets: [@liveshops/design-tokens/tailwind-preset]` 和 `content`。preset 的每一档色阶、圆角、阴影、字体都指回 `tokens.css`，因此 `bg-primary`、`rounded-lg` 和手写的 `var(--ls-*)` 不可能对不上。
- 颜色、字体、间距、圆角等跨端变量来自 `@liveshops/design-tokens`；禁止复制一套近似 Design Token，也禁止在 `theme.extend` 里另写色值。
...
| 公共协议 | `liveshop-protocol/<module>/proto/<module>`、所属 surface 的 `controller/grpc` | Proto 字段号、生成文件 |
