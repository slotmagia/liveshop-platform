...
func (r *ReleaseRepository) Activate(ctx context.Context, actor *model.RegistryAuditActor, moduleID, version string) error {
	return transaction(ctx, r.db, registryWriteTimeout, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err := persistRegistryState(ctx, tx, state); err != nil {
			return err
		}
		return auditRegistry(ctx, tx, actor, "registry.module.activate", moduleID, map[string]string{"version": version})
	})
}
...
func (r *ReleaseRepository) Deactivate(ctx context.Context, actor *model.RegistryAuditActor, moduleID string) error {
	return transaction(ctx, r.db, registryWriteTimeout, func(ctx context.Context, tx *sql.Tx) error {
		...
		version := state.ActiveVersion(moduleID)
		...
		return auditRegistry(ctx, tx, actor, "registry.module.deactivate", moduleID, map[string]string{"version": version})
	})
}
...
func auditRegistry(ctx context.Context, tx *sql.Tx, actor *model.RegistryAuditActor, action, moduleID string, details any) error {
	if actor == nil {
		return nil
	}
	if !actor.Valid() {
