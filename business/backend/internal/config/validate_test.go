...
func TestIdentityGRPCWorkloadRejectsGatewayPermission(t *testing.T) {
	...
	cfg.WorkloadIdentity.GRPC.Identity.Permissions = []string{"platform.registry.active-capabilities.read", "platform.registry.routes.read"}
	if err := validateWorkloadIdentity(cfg); err == nil {
...
func completeWorkloadConfig() *Config {
	...
	cfg.WorkloadIdentity.GRPC.Identity = MTLSWorkload{SPIFFEID: "spiffe://liveshop.local/identity", Subject: "identity", Permissions: []string{"platform.registry.active-capabilities.read"}}
	return cfg
}
