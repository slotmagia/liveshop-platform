...
func serverCredentials(config TLSConfig) (credentials.TransportCredentials, error) {
	...
	if certificateFile == "" || privateKeyFile == "" || clientCAFile == "" {
		return nil, fmt.Errorf("platform gRPC TLS certificate_file, private_key_file and client_ca_file are required")
	}
	certificate, err := tls.LoadX509KeyPair(certificateFile, privateKeyFile)
	if err != nil {
		return nil, fmt.Errorf("platform gRPC load server certificate: %w", err)
	}
	clientCAPEM, err := os.ReadFile(clientCAFile)
	if err != nil {
		return nil, fmt.Errorf("platform gRPC read client CA: %w", err)
	}
	clientCAs := x509.NewCertPool()
	if !clientCAs.AppendCertsFromPEM(clientCAPEM) {
		return nil, fmt.Errorf("platform gRPC client CA does not contain a certificate")
	}
