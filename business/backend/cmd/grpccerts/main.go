...
func generate(output string, force bool, owner int) error {
	...
	if info, err := os.Stat(output); err == nil && info.IsDir() {
		...
		if len(entries) > 0 && !force {
			...
			fmt.Printf("reusing local Platform gRPC certificates in %s\n", output)
			return nil
		}
	}
...
func validateExistingBundle(output string, now time.Time) error {
	for _, name := range []string{"ca.pem", "server.pem", "server-key.pem", "identity-server.pem", "identity-server-key.pem", "gateway.pem", "gateway-key.pem", "release.pem", "release-key.pem", "platform-client.pem", "platform-client-key.pem", "identity-client.pem", "identity-client-key.pem", "catalog.pem", "catalog-key.pem"} {
