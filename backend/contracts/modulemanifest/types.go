...
var validAuthentication = map[string]bool{"module-session": true, "workload-identity": true, "public": true}
...
// grants that capability to a user; grants are owned by the platform IAM.
...
func (m Manifest) Validate() error {
	...
	for _, route := range m.Spec.Backend.HTTPRoutes {
		...
		for _, operation := range route.Operations {
			...
			if operation.Authentication != "public" && len(operation.RequiredPermissions) == 0 {
	...
	for _, contribution := range m.Spec.Contributions {
		...
		if len(contribution.AllowedRoutes) == 0 {
			return fmt.Errorf("contribution %s must declare allowed routes", contribution.ID)
		}
		if len(contribution.RequiredPermissions) == 0 {
			return fmt.Errorf("contribution %s must declare required permissions", contribution.ID)
		}
		...
		for _, allowed := range contribution.AllowedRoutes {
			...
			if len(allowed.RequiredPermissions) == 0 {
				return fmt.Errorf("contribution %s route must declare required permissions", contribution.ID)
			}
