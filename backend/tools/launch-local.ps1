...
foreach ($candidate in $profiles) {
  $ports = @(
    ...
    (5173+$candidate.FrontendOffset), (5174+$candidate.FrontendOffset),
...
$merchUrl = "http://127.0.0.1:$(5174+$profile.FrontendOffset)"
$shopUrl = "http://127.0.0.1:$(5175+$profile.FrontendOffset)"
$liveUrl = "http://127.0.0.1:$(5176+$profile.FrontendOffset)"
$iamUrl = "http://127.0.0.1:$(5180+$profile.FrontendOffset)"
...
$accessToken = & go -C $kernelRoot run ./cmd/identitytoken
if ($LASTEXITCODE -ne 0 -or -not $accessToken) { throw 'Failed to issue local access identity.' }

try {
  & docker compose -f (Join-Path $root 'backend/deploy/compose.local.yml') up -d --wait registry-db
  if ($LASTEXITCODE -ne 0) { throw 'Failed to start the PostgreSQL module registry.' }
  ...
  & docker compose -f (Join-Path $root 'backend/deploy/compose.local.yml') run --rm registry-seed-local
  if ($LASTEXITCODE -ne 0) { throw 'Failed to seed the local IAM administrator.' }
  ...
  Wait-LocalUrl $iamUrl
  & (Join-Path $PSScriptRoot 'register-platform-module.ps1') -PlatformUrl $platformUrl -GRPCEndpoint "dns:///127.0.0.1:$($profile.PlatformGRPC)" -ArtifactUrl $iamUrl
  if ($LASTEXITCODE -ne 0) { throw 'Failed to register the Platform IAM module.' }
  $null = Invoke-WebRequest -Uri "$gatewayUrl/runtime/v1/contributions?surface=shop" -Headers @{Authorization="Bearer $accessToken";'X-Liveshop-Surface'='shop'} -TimeoutSec 3
  Wait-LocalUrl "$gatewayUrl/health"
  Wait-LocalUrl $adminUrl
  Wait-LocalUrl $merchUrl
  ...
  [ordered]@{
    ...
    adminUrl=$adminUrl; merchUrl=$merchUrl; shopUrl=$shopUrl; liveUrl=$liveUrl; iamUrl=$iamUrl; accessToken=$accessToken
  ...
  Write-Host "  Merchant: $merchUrl"
  Write-Host "  Shop:     $shopUrl"
  Write-Host "  Live:     $liveUrl"
  Write-Host "  IAM artifact: $iamUrl"
  Write-Host "  Platform: $platformUrl"
  Write-Host "  Platform gRPC: 127.0.0.1:$($profile.PlatformGRPC)"
  Write-Host "  Gateway:  $gatewayUrl"
  Write-Host '  Platform login: admin / admin (local development only)'
  Write-Host '  Merchant login: merch@sufeipay.com / 123456 (local development only)'
